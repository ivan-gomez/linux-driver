								...
								loff_t *offset)
{
	int ret = 0;
	pr_info("[%d] %s\n", __LINE__, __func__);
	pr_info("[%d] nbuf = %d, offset = %d", __LINE__,
		(int)nbuf, (int)*offset);
	/*If offset is greater that size of kbuffer, there is nothing to copy*/
	if (*offset >= kbuffer_size) {
		pr_info("[%d] Offset greater or equal than kbuffer size: %d >= %ld",
...
/*
 * Function called when the user side
 * application closes a handle to mydev driver
...
long device_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
	...
	case MYCALC_IOC_SET_NUM1:
		copy_from_user(&num1, (int *) arg, 4);
		break;
	case MYCALC_IOC_SET_NUM2:
		copy_from_user(&num2, (int *) arg, 4);
		break;
	case MYCALC_IOC_SET_OPERATION:
		copy_from_user(&op, (int *) arg, 4);
		operador = arg;
		break;
	case MYCALC_IOC_SET_RESULT:
		switch (op) {
		case 1:
			result = sum(num1, num2);
			break;
...
MODULE_AUTHOR("Equipo 2: Braian Higuera and Angel Acosta");
MODULE_LICENSE("GPL");
